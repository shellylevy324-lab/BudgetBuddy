...
const session=window.BuddySkillFramework?.createSession({studentId:student?.id||"local-student",activityKey:"money-identification",activityName:"Money Identification",promptingMode:shared.promptingMode,moduleVersion:"3.0.5",reinforcementPackageId:String(settings.reinforcement_package||"").startsWith("library:")?String(settings.reinforcement_package).slice(8):null});
...
function imageCard(coin,side,button=false){const el=document.createElement(button?"button":"div");el.className=`coin-photo-card ${button?"coin-choice":""}`;if(button)el.type="button";const scale=Math.max(.65,Math.min(1,Number(coin.diameterMm||24.26)/24.26));el.style.setProperty("--coin-scale",scale.toFixed(3));el.dataset.coinSize=coin.id;const img=document.createElement("img");img.src=coin[side];img.alt=`${coin.name}, ${side}, shown at its relative size compared with the other coins`;img.loading="eager";el.appendChild(img);return el}
...
$("startSession").addEventListener("click",start);$("endEarly").addEventListener("click",finish);$("returnToActivities").addEventListener("click",()=>location.replace("training-station.html?v=3.0.5"));
