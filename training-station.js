...
async function loadStudentHome(){
  ...
  const settings=window.BuddySkillFramework?.settingsFrom(selectedStudent)||selectedStudent.instructionalSettings||{};
  const access=settings.activity_access||{};
  const activities=(window.BuddySkillRegistry?.available()||[]).filter(activity=>access[activity.key]!==false);
...
async function redeemStudentAccess(code){
  ...
  if(!window.supabase?.createClient||!cfg?.url||!cfg?.publishableKey)throw new Error("Student access service is unavailable.");
  const client=window.supabase.createClient(cfg.url.trim(),cfg.publishableKey.trim());
  const {data,error}=await client.rpc("redeem_student_access",{p_access_code:String(code).trim().toUpperCase()});
